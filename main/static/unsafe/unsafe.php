...
<body class="p-3 mb-2 bg-danger text-white">
    <div id="content">
        ...
        <p>
            <?php echo ($url) ?> は，<b><?php echo ($ex_url) ?></b>
            へリダイレクトしようとしています．このサイトでは，悪意のあるユーザーによって，個人情報（パスワード，電話番号，クレジットカード情報 など）の入力といった危険な操作を行うよう誘導される可能性があります．
        </p>

        <button type="button" class="btn btn-outline-light" onclick="location.href='<?php echo ($ex_url) ?>'">リスクを承知でアクセスする</button>
        <button type="button" class="btn btn-outline-light" onclick="location.href='https://bitlinker.usay05.com'">安全なページへ戻る</button>
